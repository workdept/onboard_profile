...
class EntityReference_SelectionHandler_AdminViews implements EntityReference_SelectionHandler {
  ...
  protected function __construct($field, $instance) {
    ...
    $this->is_admin = in_array('administrator', $user->roles);
    $this->view_key = ($this->is_admin ? 'admin_view' : 'view');
  }
  ...
  public static function settingsForm($field, $instance) {
    ...
    $admin_view_settings = empty($field['settings']['handler_settings']['admin_view']) ? '' : $field['settings']['handler_settings']['admin_view'];
    ...
    // ideal).
    $form['view']['#element_validate'] = array('onboard_types_view_settings_validate');
    $form['admin_view']['#element_validate'] = array('onboard_types_view_settings_validate');
    ...
    if ($options) {
      ...
      $admin_default = !empty($admin_view_settings['view_name']) ? $admin_view_settings['view_name'] . ':' . $admin_view_settings['display_name'] : NULL;
      $form['admin_view']['view_and_display'] = array(
        '#type' => 'select',
        '#title' => t('View used to select the entities for administrators'),
        '#required' => TRUE,
        '#options' => $options,
      ...
      $admin_default = !empty($admin_view_settings['args']) ? implode(', ', $admin_view_settings['args']) : '';
      $form['admin_view']['args'] = array(
        '#type' => 'textfield',
        '#title' => t('View arguments'),
        '#default_value' => $admin_default,
        '#required' => FALSE,
      ...
      $form['admin_view']['no_view_help'] = array(
        '#markup' => '<p>' . t('No eligible views were found. <a href="@create">Create a view</a> with an <em>Entity Reference</em> display, or add such a display to an <a href="@existing">existing view</a>.', array(
          '@create' => url('admin/structure/views/add'),
          '@existing' => url('admin/structure/views'),
        )) . '</p>',
  ...
  protected function initializeView($match = NULL, $match_operator = 'CONTAINS', $limit = 0, $ids = NULL) {
    $view_name = $this->field['settings']['handler_settings'][$this->view_key]['view_name'];
    $display_name = $this->field['settings']['handler_settings'][$this->view_key]['display_name'];
    $args = $this->field['settings']['handler_settings'][$this->view_key]['args'];
...
function onboard_types_view_settings_validate($element, &$form_state, $form) {
